use cpal::{traits::*, EventLoop, Host, SampleFormat, StreamId};
		...
		for (i, device) in devices.enumerate() {
			...
			for format in formats {
				log::info!(
					"\t\t{}ch ({} - {}) {:?}",
					format.channels,
					format.min_sample_rate.0,
					format.max_sample_rate.0,
					format.data_type
				);
	...
	fn init(
		...
		let stream_id = {
			...
			if device_index >= fitting_device_indices.len() {
				log::warn!(
					"Index {} is bigger than number of fitting devices ({}). Using 0.",
					device_index,
			...
			let device = host
				.devices()
				.unwrap()
				.nth(fitting_device_indices[device_index])
				.unwrap();
